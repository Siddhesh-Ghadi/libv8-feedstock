...
if [[ "${target_platform}" == "osx-64" ]]; then
  ...
  if [[ "${target_platform}" == "linux-ppc64le" ]]; then
    sed -i "s/ gcc/ powerpc64le-conda_cos7-linux-gnu-gcc/g" out.gn/toolchain.ninja
    sed -i "s/ g++/ powerpc64le-conda_cos7-linux-gnu-g++/g" out.gn/toolchain.ninja
    sed -i 's/deps = powerpc64le-conda_cos7-linux-gnu.*$//g' out.gn/toolchain.ninja
  else
